          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <!-- 服务器会提供高亮选择，因为被改了，就自己写 -->
                <dd
                  changepirce="0"
                  :class="{ active: skuSaleAttr.spuId == 1 }"
                  @click="changeActive(skuSaleAttr, 1)"
                >
                  这里被删掉啦！
                </dd>
                <dd
                  changepirce="0"
                  :class="{ active: skuSaleAttr.spuId != 1 }"
                  @click="changeActive(skuSaleAttr, 2)"
                >
                  多给一个选择吧
                </dd>
              </dl>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 这里在路由跳转之前，需要把购买的产品通过请求的形式通知服务器，服务器进行存储，所以不直接进行路由跳转 -->
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      // return this.skuInfo.skuImageList ? this.skuInfo.skuImageList : [];
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    changeActive(skuSaleAttr, id) {
      // console.log(this.skuSaleAttrValueList);
      // 这里应该采用排他思想的
      if (id == 1) {
        skuSaleAttr.spuId = 1;
      } else skuSaleAttr.spuId = 0;
    },
    changeSkuNum(e) {
      // 视频写法，不能在input中直接type=number，因为还是会有小数
      ...
      const reg = /^[1-9][0-9]$/;
      if (reg.test(value)) this.skuNum = value;
      else {
        alert("输入错误！");
        this.skuNum = 1;
      }
    ...
    async addShopCar() {
      ...
      // 失败，给用户进行提示
      try {
        await this.$store.dispatch("detail/getShopCar", {
          skuNum: this.skuNum,
          skuId: this.$route.params.skuId,
        });
        // 还需要把产品信息参数传过去，可以传query参数，但是不建议，建议使用本地存储
      ...
      // console.log(result); //用了await，result不是promise了，不能用then
      // result.then(
      //   (value) => {
